...
figure('Position', [100, 100, 1366, 600]);
subplot(1,3,1)
...
for i=1:6
    ...
    if i == 1 || i ==2
        fs_color=[27 158 119]/255;
        fs_marker='o';
    elseif i == 3 || i ==4
        fs_color=[217 95 2]/255;
        fs_marker='o';
    elseif i == 5 || i ==6
        fs_color=[117 112 179]/255;
        fs_marker='o';
    ...
    y_actual=yaw_60(i,:);
    y_pred=polyval(p,roll_pow_ang);
    SSres = sum((y_actual - y_pred).^2);
    SStot = sum((y_actual - mean(y_actual)).^2);
    R2(i) = 1 - SSres/SStot;

    plot(x,y,strcat(speed_line),'Color',fs_color,'LineWidth',LineWidth)
    plot(roll_pow_ang,yaw_60(i,:),strcat(fs_marker,fs_color),...
        'MarkerFaceColor',fs_color,'MarkerEdgeColor','k','MarkerSize',MarkerSize)
    
    [val,ind]=max(y);
    x_ind=interp1([0,length(y)],[0,90],ind);
    xline(x_ind,speed_line,'Color',fs_color)
...
for i=1:6
    ...
    y=polyval(p,x);
    plot(x,y,strcat(speed_line),'Color',fs_color,'LineWidth',2)
