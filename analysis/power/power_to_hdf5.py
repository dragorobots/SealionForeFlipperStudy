#!/usr/bin/env python3
"""
Convert Power stroke .mat files to GUI-compatible HDF5 format.

This script processes Power stroke experimental data from .mat files and converts
...
def _load_mat(path: str) -> Dict[str, Any]:
    ...
    if mat73 is None:
        raise ImportError("mat73 is required for Power stroke .mat files")
...
def _find_trials_power_layout(d: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Handle the Power stroke layout: results['data']['data'] is list of trials,
    results['parameters']['parameters'] contains parameter arrays.
    """
    trials: List[Dict[str, Any]] = []
    
    # Power stroke structure: results['data']['data'] is the trials list
    data_container = d.get('data', {})
    if isinstance(data_container, dict):
        data_arr = data_container.get('data')
    else:
        data_arr = data_container
    
    # Parameters are nested: results['parameters']['parameters']
    params_container = d.get('parameters', {})
    if isinstance(params_container, dict):
        params_arr = params_container.get('parameters')
    else:
        params_arr = params_container
    ...
    for i in range(n):
        # Power stroke: trial data is numpy array (7500, 3)
        ...
        # Create trial dict for Power stroke data
        tr: Dict[str, Any] = {
            'time': time_vector,
            'lift': lift_data, 
            'thrust': thrust_data,
...
def process_files(paths: List[str], out_path: str, resample_n: int = 1001) -> None:
    """Process Power stroke .mat files and save to HDF5."""
    ...
    for path in paths:
        ...
        try:
            ...
            trials = _find_trials_power_layout(results)
    ...
    print(f"Saved Power stroke data to {out_path}")


def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Convert Power stroke .mat to GUI‑compatible HDF5.")
