...
Interactive GUI for visualizing power-stroke experimental data with the following encodings:
- X-axis: roll angle
- Columns: yaw amplitude  
- Color: flow speed
...
class PowerStrokeOverviewGUI(QMainWindow):
    ...
    def _param_map(self, params):
        """Normalize raw HDF5 parameters to canonical terms for Power stroke data."""
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("Power-Stroke Overview Metrics GUI")
    ...
    def auto_load_data(self):
        """Automatically load the Power stroke aligned trial data"""
        # Look for the power_stroke_aligned_trials file
        possible_paths = [
            "data/processed/power_stroke_aligned_trials.h5",
            "power_stroke_aligned_trials.h5"
        ]
        ...
        QMessageBox.warning(self, "Data Not Found", 
                           "Could not find power_stroke_aligned_trials.h5 file.\n"
                           "Please run the Power stroke trial processing script first.")
        self.statusBar().showMessage("Data file not found")
        
    def load_trial_traces_data(self):
        """Load the Power stroke aligned trial data from HDF5 file"""
        try:
            with h5py.File(self.trial_traces_file, 'r') as f:
                ...
                # Load all experiments (Power stroke structure: experiment_000, experiment_001, etc.)
                for exp_key in f.keys():
                    if exp_key.startswith('experiment_'):
                        ...
                        params = {
                            'period': exp_group.attrs['period'],
                            'yaw_amplitude': exp_group.attrs['yaw_amplitude'],
                            'roll_angle': exp_group.attrs['roll_angle'],
                            'flow_speed': exp_group.attrs['flow_speed']
                        }
                        
                        # Load mean traces
                        thrust_mean = np.array(exp_group['thrust_mean'])
                        lift_mean = np.array(exp_group['lift_mean'])
    ...
    def plot_overlay(self):
        ...
        try:
            ...
            print(f"DEBUG: Found {len(selections)} selections for plotting")
            if len(selections) == 0:
                print("DEBUG: No selections found, nothing to plot")
                return
            ...
            for exp_key, flow, period, yaw, roll in reversed(selections):
                print(f"DEBUG: Processing experiment {exp_key}")
                if exp_key not in self.data['experiments']:
                    print(f"DEBUG: Experiment {exp_key} not found in data")
                    continue
    ...
    def get_experiments_for_parameter(self, variable_type, param_value):
        ...
        for exp_key, exp_data in self.data['experiments'].items():
            ...
            if variable_type == "flow":
                ...
                # For sweep: fix flow=0.1, period=2.25, vary sweep
                if (abs(m['flow'] - 0.1) < 1e-6 and
                    abs(m['stroke_period'] - 2.25) < 1e-6 and
