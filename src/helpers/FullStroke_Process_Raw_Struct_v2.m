function FullStroke_Process_Raw_Struct_v2()
% FullStroke_Process_Raw_Struct_v2 - Process and prune Full Stroke data
% 
...
fprintf(fid, '=== FULL STROKE DATA PROCESSING ===\n');
fprintf(fid, 'Processing and pruning Full Stroke datasets...\n\n');

% Create output directory
output_dir = 'data/processed/2025-01-27_ProcessedData';
if ~exist(output_dir, 'dir')
    mkdir(output_dir);
    fprintf(fid, 'Created output directory: %s\n', output_dir);
end
...
fprintf(fid, 'Loading Full Stroke datasets...\n');

try
    % Load 20-Jan dataset (Flow speed 0.1 m/s)
    fprintf(fid, '  Loading 20-Jan-2023 dataset...\n');
    load('20-Jan-2023_results_FullStroke.mat');
    data_20Jan = results;
    fprintf(fid, '    Loaded: 20-Jan dataset with %d experiments\n', length(data_20Jan.data));
    
    % Load 30-Jan dataset (Flow speed 0 m/s)
    fprintf(fid, '  Loading 30-Jan-2023 dataset...\n');
    load('30-Jan-2023_results_FullStroke.mat');
    data_30Jan = results;
    fprintf(fid, '    Loaded: 30-Jan dataset with %d experiments\n', length(data_30Jan.data));
    
catch ME
    fprintf(fid, 'Error loading datasets: %s\n', ME.message);
    fclose(fid);
    return;
end
...
fprintf(fid, '  Period: %.2f s\n', target_params.period);
fprintf(fid, '  Yaw amplitudes: %s degrees\n', mat2str(target_params.y_amp));
fprintf(fid, '  Paddle transitions: %s\n', mat2str(target_params.paddle_trans));
fprintf(fid, '  Flow speeds: %s m/s\n', mat2str(target_params.flow_speeds));

%% Process 20-Jan dataset (Flow speed 0.1 m/s)
fprintf(fid, '\n=== PROCESSING 20-JAN DATASET (Flow speed 0.1 m/s) ===\n');
[pruned_20Jan, valid_indices_20] = prune_dataset(data_20Jan, target_params, 0.1, fid);
fprintf(fid, '20-Jan dataset: %d/%d experiments selected\n', length(valid_indices_20), length(data_20Jan.data));

%% Process 30-Jan dataset (Flow speed 0 m/s)
fprintf(fid, '\n=== PROCESSING 30-JAN DATASET (Flow speed 0 m/s) ===\n');
[pruned_30Jan, valid_indices_30] = prune_dataset(data_30Jan, target_params, 0, fid);
fprintf(fid, '30-Jan dataset: %d/%d experiments selected\n', length(valid_indices_30), length(data_30Jan.data));
...
fprintf(fid, 'Adding 20-Jan data (Flow speed 0.1 m/s)...\n');
...
fprintf(fid, 'Adding 30-Jan data (Flow speed 0 m/s)...\n');
...
%% Display summary
fprintf(fid, '\n=== PROCESSING SUMMARY ===\n');
fprintf(fid, 'Total experiments in combined dataset: %d\n', combined_data.metadata.total_experiments);
fprintf(fid, '  From 20-Jan (0.1 m/s): %d experiments\n', combined_data.metadata.experiments_from_20Jan);
fprintf(fid, '  From 30-Jan (0 m/s): %d experiments\n', combined_data.metadata.experiments_from_30Jan);
...
fprintf(fid, '  Roll angles: %s degrees\n', mat2str(unique(roll_angles)));

%% Save the processed data
output_filename = fullfile(output_dir, 'FullStroke_Processed_2025-01-27.mat');
fprintf(fid, '\nSaving processed data to: %s\n', output_filename);

try
    save(output_filename, 'combined_data', '-v7.3');
    fprintf(fid, 'Processing complete!\n');
    fprintf(fid, 'Output saved to: %s\n', output_filename);
catch ME
    fprintf(fid, 'Error saving data: %s\n', ME.message);
end
...
function [pruned_data, valid_indices] = prune_dataset(data_struct, target_params, flow_speed, fid)
% Prune dataset based on target parameters
% Inputs:
...
fprintf(fid, '  Pruning dataset with flow speed %.1f m/s...\n', flow_speed);
...
fprintf(fid, '    Found %d valid experiments out of %d total\n', length(valid_indices), num_experiments);
