function FullStroke_Process_Raw_Struct()
% FullStroke_Process_Raw_Struct - Process and prune Full Stroke data
% 
...
%
% Output: Pruned data structure with matching indices across data, zeros, and parameters

clear all
clc
close all

fprintf('=== FULL STROKE DATA PROCESSING ===\n');
...
var_name_20 = who;
data_20Jan = eval(var_name_20{1});
fprintf('    Loaded: %s with %d experiments\n', var_name_20{1}, length(data_20Jan.exp_num));

% Load 30-Jan dataset (Flow speed 0 m/s)
fprintf('  Loading 30-Jan-2023 dataset...\n');
load('30-Jan-2023_results_FullStroke.mat');
var_name_30 = who;
data_30Jan = eval(var_name_30{1});
fprintf('    Loaded: %s with %d experiments\n', var_name_30{1}, length(data_30Jan.exp_num));
...
[pruned_20Jan, valid_indices_20] = prune_dataset(data_20Jan, target_params, 0.1);
fprintf('20-Jan dataset: %d/%d experiments selected\n', length(valid_indices_20), length(data_20Jan.exp_num));

%% Process 30-Jan dataset (Flow speed 0 m/s)
fprintf('\n=== PROCESSING 30-JAN DATASET (Flow speed 0 m/s) ===\n');
[pruned_30Jan, valid_indices_30] = prune_dataset(data_30Jan, target_params, 0);
fprintf('30-Jan dataset: %d/%d experiments selected\n', length(valid_indices_30), length(data_30Jan.exp_num));
...
combined_data.metadata.total_experiments = length(fieldnames(combined_data.data));
...
function [pruned_data, valid_indices] = prune_dataset(data_struct, target_params, flow_speed)
