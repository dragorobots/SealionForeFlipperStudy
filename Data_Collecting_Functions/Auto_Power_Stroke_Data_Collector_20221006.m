...
roll_pow_ang_settings=[-90,-75,-60,-55,-50,-45,-40,-35,-30,-15,0];
%roll_pow_ang_settings=[0];
Flow_Speed_settings=[100,0];
...
disp('')
disp('')
disp('Estimated Time of Experiments:')
disp(estimated_time)
disp('')
disp('')
%% Results Set up

results.experiment_type='Power Stroke Study';
results.fin_type='Flipper_A';
...
results.date=date;
results.sensor_order={'Lift on ai0','Thrust on ai2','Arduino on ai4'};
...
z=1;
    ...
    disp('Flow Tank Ard, reset and Connected...')
...
for motor_power=Flow_Speed_settings
    ...
    for Period=period_settings
        for y_amp=y_amp_settings
            for roll_pow_ang=roll_pow_ang_settings
                ...
                [pitch,yaw,roll,~]=flipper_trajs_simulation_2(Num_Pts,Period, ...
                ...
                %% Separtating Power Stroke and creating return

                clear pitch_pow yaw_pow roll_pow
                pitch_pow=  pitch(Num_Pts*pitch_power_start:Num_Pts*pitch_power_end);
                yaw_pow  =  yaw(Num_Pts*pitch_power_start:Num_Pts*pitch_power_end);
                roll_pow =  roll(Num_Pts*pitch_power_start:Num_Pts*pitch_power_end);

                pow_leng=length(pitch_pow);
                pitch_return=linspace(max(pitch_pow),min(pitch_pow),100);
                pitch_return=[linspace(max(pitch_pow),max(pitch_pow),100),...
                    pitch_return, ...
                    linspace(min(pitch_pow),min(pitch_pow),200-length(pitch_pow))];
                pitch_pow=[pitch_pow,pitch_return];

                yaw_return=linspace(min(yaw_pow),max(yaw_pow),100);
                yaw_return=[linspace(min(yaw_pow),min(yaw_pow),100),...
                    yaw_return,...
                    linspace(max(yaw_pow), max(yaw_pow),200-length(yaw_pow))];
                ...
                results.parameters(z).parameters=[Period,y_amp,roll_pow_ang,motor_power];
                ...
                figure('Renderer', 'painters', 'Position', [1200 700 600 300])
                subplot(3,1,1)
                plot(data(:,1))
                title('Lift')
    ...
    save(strcat(directory_name,"\",date,"_results_PowerStroke.mat"),'results','-v7.3')
...
%%
    ...
    save(strcat(directory_name,"\",date,"_videos_PowerStroke.mat"),'videos','-v7.3')
