...
Flow_Speed_settings=[70,0];

total_exps=length(period_settings)*length(y_amp_settings)*length(roll_paddle_ang_settings)*length(Flow_Speed_settings);
...
% results.roll_pow_ang_settings=roll_paddle_ang_settings;
% results.Flow_Speed_settings=Flow_Speed_settings;
% results.date=date;
% results.sensor_order={'Thrust on ai0','Lift on ai2','Arduino on ai4'};
...
for motor_power=0
    ...
    for Period=period_settings
        for y_amp=y_amp_settings
            for roll_paddle_ang=roll_paddle_ang_settings
